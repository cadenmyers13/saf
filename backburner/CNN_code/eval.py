...
def process_gr_input_single(r, g):
    """Reprocess the PDF into PDF input for the CNN model. The feature input to
    the CNN is a 209 x 1 array, with r-range from 1.5 A to 30. A.
    ...
    CNN_R_RANGE = np.linspace(1.5, 30.0, 209)
    input_pdf = pdf_interp(CNN_R_RANGE)

    # NORMALIZE
    min_g = np.amin(input_pdf)
    max_g = np.amax(input_pdf)
    norm_input = 1.0 / (max_g - min_g) * (input_pdf - min_g)
    return norm_input
...
def predict_sg(r, g):
    ...
    model = models.load_model(
        MODEL_PATH, custom_objects={"top6_acc": acc_metric}
    )
    ...
    output_dict = {}
    output_dict["space_group"] = np.array(
        [
            "P-1",
            "P2_1",
            "Cc",
    ...
    output_dict["space_group_number"] = np.array(
        [
            "2",
            "4",
            "9",
    ...
    results_df_sorted = results_df.sort_values(
        by=["space_group_number"], ascending=False
    )
